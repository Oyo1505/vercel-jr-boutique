...
export default function ProductGridItems({ products, limit=2 }: { products: Product[], limit:number }) {
  const pathname = usePathname();

  return (
    <Suspense>
      {products.map((product, index) => (
        index <= limit && <Grid.Item key={product.handle} className={styles.gridItem}>
